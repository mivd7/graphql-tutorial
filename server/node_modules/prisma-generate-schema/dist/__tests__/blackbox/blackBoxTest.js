...
var datamodel_1 = require("../../src/datamodel");
function blackBoxTest(name, databaseType) {
    var generator = generator_1.default.create(databaseType);
    var modelPath = path.join(__dirname, "cases/" + name + "/model_" + databaseType + ".graphql");
    var prismaPath = path.join(__dirname, "cases/" + name + "/" + databaseType + ".graphql");
    ...
    var types = datamodel_1.default.create(databaseType).parseFromSchemaString(model);
    ...
    fs.writeFileSync(path.join(__dirname, "cases/" + name + "/generated_" + databaseType + ".graphql"), ourPrintedSchema, { encoding: 'UTF-8' });
    // Check if our schema equals the prisma schema.
    var prismaSchema = utilities_1.buildSchema(prisma);
    astTools_1.default.assertTypesEqual(prismaSchema, ourSchema, name + "/" + databaseType);
...
var _loop_1 = function (testName) {
    test("Generates schema for " + testName + "/relational correctly", function () {
        blackBoxTest(testName, databaseType_1.DatabaseType.relational);
    });
    test("Generates schema for " + testName + "/document correctly", function () {
        blackBoxTest(testName, databaseType_1.DatabaseType.document);
    });
};
