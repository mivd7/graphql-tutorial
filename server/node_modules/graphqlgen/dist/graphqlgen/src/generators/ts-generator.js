...
function renderScalarResolvers(type, modelMap) {
    ...
    return "export const defaultResolvers = {\n    " + interfaceChildNodes
        ...
        .filter(function (fieldName) { return type.fields.some(function (field) { return field.name === fieldName; }); })
        .map(function (fieldName) { return renderScalarResolver(fieldName, model.modelTypeName); })
        .join(os.EOL) + "\n  }";
}
function renderScalarResolver(fieldName, parentTypeName) {
...
function getModelName(typeName, modelMap) {
    var model = modelMap[typeName];
...
function printFieldLikeType(field, modelMap) {
    ...
    return "" + getModelName(field.type.name, modelMap) + (field.type.isArray ? '[]' : '') + (!field.type.isRequired ? '| null' : '');
}
