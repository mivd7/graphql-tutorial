...
function generateCode(generateCodeArgs) {
    var schema = generateCodeArgs.schema;
    var generateArgs = __assign({}, schema, { context: parse_1.parseContext(generateCodeArgs.config.context, generateCodeArgs.config.output), modelMap: generateCodeArgs.modelMap });
...
function writeResolversScaffolding(resolvers, config) {
    ...
    resolvers.forEach(function (f) {
        ...
        try {
            fs.writeFileSync(writePath, utils_1.replaceAll(f.code, '[TEMPLATE-INTERFACES-PATH]', path_helpers_1.getImportPathRelativeToOutput(path_helpers_1.getAbsoluteFilePath(config.output, config.language), writePath)));
        }
        catch (e) {
